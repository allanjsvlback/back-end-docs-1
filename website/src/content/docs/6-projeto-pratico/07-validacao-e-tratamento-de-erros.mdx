...
Para resolver isso, vamos usar um dos recursos mais legais do Nest.js: os **Pipes**, especificamente o `ValidationPipe`, para validar automaticamente os dados de entrada com base nos nossos DTOs.
...
4.  **Entendendo o Tratamento de Erros**
    ...
    Este mecanismo é muito massa. A maior parte do tempo, você só precisará lançar as exceções HTTP padrão do `@nestjs/common` (como `NotFoundException`, `ConflictException`, `UnauthorizedException`) e o Nest cuidará de formatar a resposta de erro para você.
